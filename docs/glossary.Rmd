...
**Definition**: Out-degree indicates the number of connections leaving from each planning unit [@Minor2007-fb]. This metric ignores edge weights or strengths, and can be calculated from [${F}$](glossary.html#flow), [${M}$](glossary.html#migration), and [${P}$](glossary.html#probability) .
...
## Connectivity as Ecological Distance

Directly substitutes the supplied connectivity data for the boundary lengths in the boundary definitions (*i.e.* ecological distance) file (*i.e.* `boundary.dat`; see @Beger2010-ir). This approach assumes the user has provided appropriate connectivity data (*e.g.* appropriate scale, units, and rank-order).
