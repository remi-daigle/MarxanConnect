...
**Definition**: Self-recruitment is the diagonal of the migration matrix, [${M}$](glossary.html#migration). It is the proportion of individuals that arrived to a planning unit that originated from that same planning unit [see @DAloia2017-jp; @Lett2015-wv]. 
...
**Definition**: Local retention is the diagonal of the probability matrix, [${P}$](glossary.html#probability), and is the proportion of those individuals originating from a planning unit that are retained within that planning unit. Therefore, this value is independent of contributions from all other planning units [@Lett2015-wv]. 
...
Here, the Edge List structure from above is duplicated for each unique type (*e.g.* species, or dispersal trait). The first column now contains the type ID, The remaining columns are exactly as Edge List, above (donor site ID, recipient site ID, and connectivity value). Type ID must contain characters (*e.g.* `sp1` not `1`)
