...
Download the example [project folder](https://minhaskamal.github.io/DownGit/#/home?url=https://github.com/remi-daigle/MarxanConnect/tree/master/../docs/tutorial/CSM_demographic). This folder contains the Marxan Connect Project file, the input data, and the output data from this example. Feel free to follow along using Marxan Connect by loading `tutorial.MarCon`.
...
```{r}
spec <- read.csv("tutorial/CSM_demographic/input/spec.dat")
...
```{r}
puvspr <- read.csv("tutorial/CSM_demographic/input/puvspr2.dat")
...
```{r}
pu <- read.csv("tutorial/CSM_demographic/input/pu.dat")
...
# planning units
pu <- st_read("tutorial/CSM_demographic/reefs.shp") %>%
    st_transform(proj)

#focus areas (IUCN level I or II protected areas)
fa <- st_read("tutorial/CSM_demographic/IUCN_IorII.shp") %>%
    st_transform(proj)

# avoidance areas (ports)
aa <- st_read("tutorial/CSM_demographic/ports.shp") %>%
    st_transform(proj)
```
...
For the sake of you web browser, this table only contains the 7 row and columns of the connectivity matrix. The real file has 321 X 321

```{r}
conmat <- read.csv("tutorial/CSM_demographic/reefFlow.csv")
datatable(conmat[1:7,1:7],rownames = FALSE, options = list(searching = FALSE))
```

### `boundary.dat`

The table shown here is a trimmed version showing the first 30 rows as an example of the type of data in the puvspr_appended.dat file. The original dataset has 57753 entries.

```{r}
puvspr <- read.csv("tutorial/CSM_demographic/input/boundary.dat")
datatable(puvspr[1:30,],rownames = FALSE, options = list(searching = FALSE))
```
...
```{r}
# planning units with output
output <- read.csv("tutorial/CSM_demographic/output/pu_connect.csv") %>%
    mutate(geometry=st_as_sfc(geometry,"+proj=longlat +datum=WGS84"),
           # best_solution = as.logical(best_solution),
