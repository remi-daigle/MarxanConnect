...
```{r}
puvspr <- read.csv("tutorial/CF_landscape/input/puvspr.dat")
...
# planning units
pu <- st_read("tutorial/CF_landscape/hex_planning_units.shp") %>% 
    st_transform(proj)
...
```{r}
conmat <- read.csv("tutorial/CF_landscape/hexFlow.csv")[1:7,1:7]
...
```{r}
puvspr <- read.csv("tutorial/CF_landscape/input/puvspr_appended.dat")
datatable(puvspr[1:30,],rownames = FALSE, options = list(searching = FALSE))
```
